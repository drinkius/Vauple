/**
 * Vault.c - An example hook for collecting signatures for multi-sign transactions without blocking sequence number
 * on the account.
 * Based on Notary.c
 *
 **/
...
#define MAX_AUTO_APPROVE 2
const uint8_t COUNTER_KEY[32] = {
    0x00U, 0x23U, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U,
...
/**
 * Vault - easy multisign with Hooks
 ...
 *    to the hook.
 * 3. Autoapprove small transactions (<=100) for MAX_AUTO_APPROVE before next signed transaction
 *
...
int64_t hook(uint32_t reserved)
{
  TRACESTR("enter hook");
    ...
    if (account_field_len < 20)                                   // negative values indicate errors from every api
        rollback(SBUF("Vault: sfAccount field missing!!!"), 10); // this code could never be hit in prod
                                                                  // but it's here for completeness

    // compare the "From Account" (sfAccount) on the transaction with the account the hook is running on
    int equal = 0; BUFFER_EQUAL(equal, hook_accid, account_field, 20);
    if (equal)
        accept(SBUF("Vault: Outgoing transaction"), 20);
    ...
    if (invoice_id_len == 32)
    {
        ...
        if (tx_len < 0)
            rollback(SBUF("Vault: Received invoice id that did not correspond to a submitted multisig txn."), 1);
        ...
        if (lls_len != 4 || UINT32_FROM_BUF(lls_ptr) < ledger_seq())
        {
            ...
            if (state_set(0, 0, SBUF(invoice_id)) < 0)
               rollback(SBUF("Vault: Error erasing old txn blob."), 40);

            accept(SBUF("Vault: Multisig txn was too old (last ledger seq passed) and was erased."), 1);
        }
    ...
    if (memos_len <= 0 && invoice_id_len <= 0)
        accept(SBUF("Vault: Incoming txn with neither memo nor invoice ID, passing."), 0);

    if (memos_len > 0 && invoice_id_len > 0)
        rollback(SBUF("Vault: Incoming txn with both memo and invoice ID, abort."), 0);
    ...
    if (util_keylet(SBUF(keylet), KEYLET_SIGNERS, SBUF(hook_accid), 0, 0, 0, 0) != 34)
        rollback(SBUF("Vault: Internal error, could not generate keylet"), 10);

    // then requesting XRPLD slot that keylet into a new slot for us
    int64_t slot_no = slot_set(SBUF(keylet), 0);
    TRACEVAR(slot_no);
    if (slot_no < 0)
        rollback(SBUF("Vault: Could not set keylet in slot"), 10);
    ...
    if (result < 0)
        rollback(SBUF("Vault: Could not find sfSignerQuorum on hook account"), 20);
    ...
    if (result != 4)
        rollback(SBUF("Vault: Could not fetch sfSignerQuorum from sfSignerEntries."), 80);
    ...
    if (result < 0)
        rollback(SBUF("Vault: Could not find sfSignerEntries on hook account"), 20);

    // since sfSignerEntries is an array type we can request its length with slot_count
    int64_t signer_count = slot_count(slot_no);
    if (signer_count < 0)
        rollback(SBUF("Vault: Could not fetch sfSignerEntries count"), 30);
    ...
    for (int i = 0; GUARD(8), i < signer_count + 1; ++i)
    {
        ...
        if (subslot < 0)
            rollback(SBUF("Vault: Could not fetch one of the sfSigner entries [subarray]."), 40);

        // load the account field from that entry into a new slot
        result = slot_subfield(subslot, sfAccount, 0);
        if (result < 0)
            rollback(SBUF("Vault: Could not fetch one of the account entires in sfSigner."), 50);

        // dump the new slot into a buffer
        uint8_t signer_account[20];
        result = slot(SBUF(signer_account), result);
        if (result != 20)
            rollback(SBUF("Vault: Could not fetch one of the sfSigner entries [slot sfAccount]."), 60);

        // load the weight field into a new slot
        result = slot_subfield(subslot, sfSignerWeight, 0);
        if (result < 0)
            rollback(SBUF("Vault: Could not fetch sfSignerWeight from sfSignerEntry."), 70);

        // dump the weight field into a buffer
        result = slot(buf, 2, result);

        if (result != 2)
            rollback(SBUF("Vault: Could not fetch sfSignerWeight from sfSignerEntry."), 80);
    ...
    if (!found)
        rollback(SBUF("Vault: Your account was not present in the signer list."), 70);
    ...
    if (memos_len > 0)
    {
        ...
        if (invoice_id_len > 0)
            rollback(SBUF("Vault: Incoming transaction with both invoice id and memo. Aborting."), 0);
        ...
        if (memo_lookup < 0)
            rollback(SBUF("Vault: Incoming txn had a blank sfMemos, abort."), 1);
        ...
        if (!is_unsigned_payload)
            accept(SBUF("Vault: Memo is an invalid format. Passing txn."), 50);
        ...
        if (data_len > MAX_MEMO_SIZE)
            rollback(SBUF("Vault: Memo too large (4kib max)."), 4);
        ...
        if (txtype_lookup < 0)
            rollback(SBUF("Vault: Memo is invalid format. Should be an unsigned transaction."), 2);
        ...
        if (lls_len != 4 || UINT32_FROM_BUF(lls_ptr) < ledger_seq() + MINIMUM_FUTURE_LEDGER)
            rollback(SBUF("Vault: Provided txn blob expires too soo (LastLedgerSeq)."), 3);
        ...
        if (util_sha512h(SBUF(invoice_id), data_ptr, data_len) < 0)
            rollback(SBUF("Vault: Could not compute sha512 over the submitted txn."), 5);
        ...
        int64_t amount_lookup = sto_subfield(data_ptr, data_len, sfAmount);
        if (amount_lookup < 0) {
            rollback(SBUF("Failed to retrieve the sfAmount subfield."), 1);
        }

        uint8_t* amount_ptr = SUB_OFFSET(amount_lookup) + data_ptr;
        uint32_t amount_len = SUB_LENGTH(amount_lookup);

        // Ensure the amount is a 64-bit integer
        if (amount_len != 8) {
            rollback(SBUF("Unexpected length for Amount field."), 1);
        }

        // // Print each byte individually for validation
        // for (int i = 0; GUARD(8), i < 8; i++) {
        //     TRACEVAR(amount_ptr[i]);
        // }
	...
	if (ssrv < 0) {
	    TRACEVAR(ssrv);
            rollback(SBUF("Vault: Could not write txn to hook state."), 6);
	}
    }
    ...
    if (state_set(buf, 2, SBUF(invoice_id)) != 2)
        rollback(SBUF("Vault: Could not write signature to hook state."), 7);
    ...
    // if it's a small amount then we can autoapprove it
    int32_t count;
    int is_auto_approved = 0;
    if (amount <= 100){
        
        if (state((uint8_t*)&count, 4, COUNTER_KEY, 32) < 0) {
            // State does not exist, initialize count to 0
            count = 0;
        }
        
        if (count < MAX_AUTO_APPROVE) {
            // Increase the count
            count++;

            // Save the updated count back to state
            if (state_set((uint8_t*)&count, 4, COUNTER_KEY, 32) < 0) {
                // Handle error setting state
                rollback(SBUF("Failed to update transaction counter."), 1);
            }
            total = signer_quorum;
            TRACESTR("Vault: Accepted transaction with amount <=100 without need to sign");
            TRACEVAR(count);
            is_auto_approved = 1;
        } else {
            TRACESTR("Vault: Max auto accepted transactions reached");
            TRACEVAR(count);
        }
    }
    ...
    {
        uint8_t header[] = "Vault: Accepted waiting for other signers...: ";
    ...
    // we reached quorum with is_auto_approved == 0 - means we should revert autoapproved to 0
    if (is_auto_approved == 0){
        count = 0;
        state_set((uint8_t*)&count, 4, COUNTER_KEY, 32);
        TRACEVAR(count);
    }
    ...
    if (!should_emit)
        rollback(SBUF("Vault: Tried to emit multisig txn but it was missing"), 1);
    ...
    uint32_t lls_len = SUB_LENGTH(lls_lookup);
    if (lls_len != 4)
        rollback(SBUF("Vault: Was about to emit txn but it doesn't have LastLedgerSequence"), 1);

    uint32_t lls_old = UINT32_FROM_BUF(lls_ptr);
    if (lls_old < ledger_seq())
        rollback(SBUF("Vault: Was about to emit txn but it's too old now"), 1);
    ...
    if (tx_len <= 0)
        rollback(SBUF("Vault: Emplacing sfSequence failed."), 1);
    ...
    TRACEVAR(tx_len);
    if (tx_len <= 0)
        rollback(SBUF("Vault: Emplacing sfTxnSignature failed."), 1);
    ...
    TRACEVAR(tx_len);
    if (tx_len <= 0)
        rollback(SBUF("Vault: Emplacing sfSigningPubKey failed."), 1);
    ...
    if (tx_len <= 0)
        rollback(SBUF("Vault: Emplacing sfFirstLedgerSequence failed."), 1);
    ...
    if (lls_old > lls_new) {
        ...
        if (tx_len <= 0)
            rollback(SBUF("Vault: Erasing sfLastLedgerSequence failed."), 1);
      
        zeroed[1] = 0x1BU;
        UINT32_TO_BUF(zeroed + 2, lls_new);
        tx_len = sto_emplace(buffer2, MAX_MEMO_SIZE, buffer1, tx_len, zeroed, 6, sfLastLedgerSequence);
        if (tx_len <= 0)
	    rollback(SBUF("Vault: Emplacing sfLastLedgerSequence failed."), 1);
    }
    ...
    if (result < 0)
        rollback(SBUF("Vault: EmitDetails failed to generate."), 1);

    tx_len = sto_emplace(buffer1, MAX_MEMO_SIZE, buffer2, tx_len, emitdet, result, sfEmitDetails);
    if (tx_len < 0)
        rollback(SBUF("Vault: Emplacing sfEmitDetails failed."), 1);
    ...
    if (fee_to_pay < 0)
        rollback(SBUF("Vault: Computing sfFee failed."), 1);
      
    ENCODE_DROPS(fee_ptr, fee_to_pay, amFEE);
    tx_len = sto_emplace(buffer2, MAX_MEMO_SIZE, buffer1, tx_len, SBUF(fee), sfFee);
    
    if (tx_len <= 0)
        rollback(SBUF("Vault: Emplacing sfFee failed."), 1);

    uint8_t emithash[32];
    int64_t erv = emit(SBUF(emithash), buffer2, tx_len);
    if (erv < 0) {
        TRACEVAR(erv);
        accept(SBUF("Vault: All conditions met but emission failed: proposed txn was malformed."), 1);
    }

    accept(SBUF("Vault: Emitted multisigned txn"), 0);
    return 0;
}
